!
! QUADPACK dqage plus dependencies taken from 
! https://people.sc.fsu.edu/~jburkardt/f_src/quadpack_double/quadpack_double.f90
!
! Wrapped and modified by Andreas van Hameren 2018-07-31.
! Modifications:
!   * real ( kind = 8 ) => real(kind(1d0))
!   * integer ( kind = 4 ) => integer
!   * "external" atributes etc. replaced by interfaces
!   * variables with name "result" renamed to "rslt"
!   * 0.5d-28 => 0.5D-28
!

module katie_quadpack
  implicit none
  private
  public :: integral

  abstract interface
    function function_interface(xx) result(rslt)
...
contains
  ...
  function integral( f ,a ,b ) result(rslt)
! Wrapper for dqage
  procedure(function_interface) :: f
  real(kind(1d0)),intent(in) :: a,b
  real(kind(1d0)) :: rslt
  real(kind(1d0)),parameter :: epsabs=0D+00 ,epsrel=1D-08 
  integer,parameter :: key=6    ! key>5: gauss-kronrod with 30-61 points
  integer,parameter :: limit=64 ! limit: maximum number of subintervals
  integer :: neval,ier,last,iord(limit)
  real(kind(1d0)) :: abserr
  real(kind(1d0)) :: alist(limit),blist(limit),elist(limit),rlist(limit)
  call dqage ( f ,a ,b ,epsabs ,epsrel ,key ,limit ,rslt ,abserr &
             ,neval ,ier ,alist ,blist ,rlist ,elist ,iord ,last )
  !write(*,*) 'abserr,neval:',abserr,neval
  end function

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
subroutine dqage ( f, a, b, epsabs, epsrel, key, limit, rslt, abserr, &
  neval, ier, alist, blist, rlist, elist, iord, last )
...
!
!      epsabs - real(kind(1d0))
!               absolute accuracy requested
!      epsrel - real(kind(1d0))
...
!
!      abserr - real(kind(1d0))
!               estimate of the modulus of the absolute error,
!               which should equal or exceed abs(i-rslt)
!
!      neval  - integer
...
!                       rslt, abserr, neval, last, rlist(1) ,
...
!
  ...
  if(key.ge.7) keyf = 6
  neval = 0
  if(keyf.eq.1) call dqk15(f,a,b,rslt,abserr,defabs,resabs)
  if(keyf.eq.2) call dqk21(f,a,b,rslt,abserr,defabs,resabs)
  if(keyf.eq.3) call dqk31(f,a,b,rslt,abserr,defabs,resabs)
  if(keyf.eq.4) call dqk41(f,a,b,rslt,abserr,defabs,resabs)
  if(keyf.eq.5) call dqk51(f,a,b,rslt,abserr,defabs,resabs)
  if(keyf.eq.6) call dqk61(f,a,b,rslt,abserr,defabs,resabs)
  last = 1
  rlist(1) = rslt
  elist(1) = abserr
  iord(1) = 1
...
!  test on accuracy.
!
  errbnd =  max ( epsabs, epsrel* abs ( rslt ) )
...
subroutine dqk15(f,a,b,rslt,abserr,resabs,resasc)
...
!        abserr - real(kind(1d0))
!                 estimate of the modulus of the absolute error,
!                 which should not exceed abs(i-rslt)
!
!        resabs - real(kind(1d0))
!                 approximation to the integral j
!
!        resasc - real(kind(1d0))
...
!
  ...
  real(kind(1d0)) a,absc,abserr,b,centr,dhlgth, &
    epmach,fc,fsum,fval1,fval2,fv1,fv2,hlgth,resabs,resasc, &
    resg,resk,reskh,rslt,uflow,wg,wgk,xgk
  integer j,jtw,jtwm1
  procedure(function_interface) :: f
...
subroutine dqk21(f,a,b,rslt,abserr,resabs,resasc)
...
subroutine dqk31(f,a,b,rslt,abserr,resabs,resasc)
...
subroutine dqk41 ( f, a, b, rslt, abserr, resabs, resasc )
...
subroutine dqk51(f,a,b,rslt,abserr,resabs,resasc)
...
subroutine dqk61(f,a,b,rslt,abserr,resabs,resasc)
...
!
  ...
  real(kind(1d0)) a,dabsc,abserr,b,centr,dhlgth, &
    epmach,fc,fsum,fval1,fval2,fv1,fv2,hlgth,resabs,resasc, &
    resg,resk,reskh,rslt,uflow,wg,wgk,xgk
  integer j,jtw,jtwm1
  procedure(function_interface) :: f
...
!
  ...
  real(kind(1d0)) elist,ermax,errmax,errmin
