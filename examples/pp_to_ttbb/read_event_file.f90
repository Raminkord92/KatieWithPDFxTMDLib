...
!== DECLARATIONS ==
implicit none
type(histo_1d_type) :: pT_bb,y_bb,m_bb
...
!== INITIALIZATIONS ==
call  m_bb%init( left=0d0 ,right=400d0 ,Nbins=100 )
call pT_bb%init( left=0d0 ,right=400d0 ,Nbins=100 )
call  y_bb%init( left=-2.5d0 ,right=2.5d0 ,Nbins=100 )
...
!== DATA COLLECTION ==
call  m_bb%collect(      mass(pFnst(0:3,3)+pFnst(0:3,4)) ,eventWeight )
call pT_bb%collect(    pTrans(pFnst(0:3,3)+pFnst(0:3,4)) ,eventWeight )
call  y_bb%collect( pseudoRap(pFnst(0:3,3)+pFnst(0:3,4)) ,eventWeight )
...
call  m_bb%write( 'm_bb.hst')
call pT_bb%write('pT_bb.hst')
call  y_bb%write( 'y_bb.hst')
