...
!== DECLARATIONS ==
implicit none
type(histo_1d_type) :: h_pT_H,h_y_H,h_pT(1:Nsize),h_y(1:Nsize)
real(kind(1d0)) :: pT(Nsize)
integer :: Njet,ii,pTordered(Nsize),iHiggs
...
enddo
call h_pT_H%init( left=0d0 ,right=200d0 ,Nbins=100 )
call h_y_H%init( left=-4.7d0 ,right=4.7d0 ,Nbins=100 )
...
enddo
call h_pT_H%collect( pTrans(pFnst(0:3,iHiggs)) ,eventWeight )
call h_y_H%collect( rapidity(pFnst(0:3,iHiggs)) ,eventWeight )
...
enddo
call h_pT_H%write('pT_H.hst')
call h_y_H%write('y_H.hst')
