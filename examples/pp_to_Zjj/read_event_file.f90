...
!== DECLARATIONS ==
implicit none
type(histo_1d_type) :: h_pT_Z,h_y_Z,h_pT(1:Nsize),h_y(1:Nsize)
...
do ii=1,Njet
  ...
  call h_y( ii)%init( left=-2.0d0 ,right=2.0d0 ,Nbins=100 )
enddo
call h_pT_Z%init( left=0d0 ,right=200d0 ,Nbins=100 )
call h_y_Z%init( left=-2.0d0 ,right=2.0d0 ,Nbins=100 )
...
!== DATA COLLECTION ==
do ii=1,Njet
  pT(ii) = pTrans(pFnst(0:3,2+ii))
enddo
call sort_big2small( pTordered ,pT ,Njet )
do ii=1,Njet
  call h_pT(ii)%collect( pT(ii) ,eventWeight )
  call h_y( ii)%collect( rapidity(pFnst(0:3,2+pTordered(ii))) ,eventWeight )
enddo
call h_pT_Z%collect( pTrans(pFnst(0:3,1)+pFnst(0:3,2)) ,eventWeight )
call h_y_Z%collect( rapidity(pFnst(0:3,1)+pFnst(0:3,2)) ,eventWeight )
...
enddo
call h_pT_Z%write('pT_Z.hst')
call h_y_Z%write('y_Z.hst')
